# Sol2 - try to simplify the circuit to obtain simplest circuit for an output - track what is incorrect there...?
def parse():
...
def solve_first(lines):
    ...
    acc = "".join(
        str(output[1]) for output in outputs
    )  # We start appending with the most significant bit
    print(len(acc))
    return acc
